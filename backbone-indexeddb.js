(function () { /*global _: false, Backbone: false */
    ...
    function Driver(schema, ready, started, options) {
        ...
        if (typeof this.options.error == "undefined") {
            this.options.error = function (e) {
                debug_error("an error occurred in backbone-indexeddb driver with error :");
                debug_error(e);
                throw e;
            }.bind(this);
        }
        ...
        //Init the different step of upgrading database schema
        ...
        this.dbRequest.onsuccess = function (e) {
            try{
                ...
                debugLog("dbRequest open succeded on version #" + this.db.version);
                ...
                else
                {
                    ...
                    if(this.isUpgradeNeeded){
                        //finir l'upgrade
                        this.onUpgradeDone(
                            _.clone(this.schema.migrations),
                            {
                                ready: function () {
                                    this.ready();
                                }.bind(this),
    ...
    Driver.prototype = {
                    ...
                    if (!version || version < migration.version) {
                        ...
                        migration.before(function () {
                            try{
                                ...
                                var continueMigration = function () {
                                    ...
                                    migration.migrate(transaction, function () {
                                        try{
                                            ...
                                            debugLog("Migrated done to version # " + migration.version);
        ...
        onUpgradeDone : function(afters,options){
            try{
                debugLog("Done migrating to db version #" + this.db.version);
                // No more migration
                options.started();
                debugLog("Begin After migrating");
                var migrationAfter =function(afters){
                    var after = afters.shift();
                    if(after){
                        debugLog("Begin After migrating on migration version #" + after.version);
                    ...
                    {
                        debugLog("All after migration success");
                        options.ready();
                    }
                }.bind(this);
        ...
        execute: function (storeName, method, object, options) {
            try
            {
                debugLog("execute : " + method +  " on " + storeName + " for " + object);
                var now = new Date();
                switch (method) {
                case "create":
...
;            }
    ...
    function ExecutionQueue(schema,next,options) {
        this.driver     = new Driver(schema, this.ready.bind(this), this.startQueue.bind(this), options);
        this.started    = false;
        this.stack      = [];
    ...
    function sync(method, object, options) {
        ...
        if (!Databases[schema.id]) {
              Databases[schema.id] = new ExecutionQueue(schema,next,options);
            }
        else{
            next();
        }
